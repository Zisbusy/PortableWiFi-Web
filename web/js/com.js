define("service", "underscore jquery set CryptoJS".split(" "), function(cg, cO, aF, bv) {
	...
	function aM() {
		...
		function c8(da, db) {
			var dc = {
				goformId: "LOGIN",
				password: aF.PASSWORD_ENCODE ? Base64.encode(da.password) : da.password
			};
			return dc
		}
	...
	function bn(c9) {
		...
		if (bu.networkType.toLowerCase()
			.indexOf("limited_service") != -1 || bu.networkType.toLowerCase()
			.indexOf("limited service") != -1) {
			bu.networkType = "limited_service"
		} else {
			if (bu.networkType.toLowerCase()
				.indexOf("no_service") != -1 || bu.networkType.toLowerCase()
				.indexOf("no service") != -1) {
	...
	function aD(da) {
		cO.post("/goform/goform_set_cmd_process", {
	...
	function aX() {
		...
		function c8(dc, dd) {
			var de = {
				...
				ID: dc.id,
				encode_type: getEncodeType(dc.message)
					.encodeType
			};
			return de
		}
	...
	function cL(c9, da, c8) {
		c4({
			...
			if (dc) {
				...
				if (db == "2") {
					c8(cO.extend(V, {
						errorType: c9.errorType,
						errorText: c9.errorText
					}))
				} else {
					if (db == "3") {
						da({
	...
	function bN() {
		...
		function c8(da, db) {
			var dc = {};
			dc.cmd = "cstm_webui_imei,cstm_webui_ttl,cstm_webui_dns,cstm_webui_bandselect,band_select_enable,ussd_enable,vpn_enable,dns_manual_func_enable,tr069_func_enable";
			dc.multi_data = 1;
			return dc
		}
	...
	function cv() {
		...
		function c8(da, db) {
			var dc = {
				...
				mode_set: da.mode == "0" ? "http_share_mode" : "usb_mode"
			};
			return dc
		}
	...
	function a9() {
		...
		function c8(db, dc) {
			...
			var dd = {
				cmd: "wifi_coverage,m_ssid_enable,imei,network_type,sub_network_type,rssi,rscp,lte_rsrp,ziccid,imsi,sim_imsi,cr_version,hw_version,MAX_Access_num," + da + "SSID1,AuthMode,m_SSID,m_AuthMode,m_HideSSID,m_MAX_Access_num,lan_ipaddr,mac_address,msisdn,LocalDomain,wan_ipaddr,static_wan_ipaddr,ipv6_wan_ipaddr,ipv6_pdp_type,pdp_type,ppp_status,sta_ip_status,rj45_state,ethwan_mode,detail_cell_rsrq,detail_cell_rssi,detail_cell_pci,detail_cell_sinr,detail_cell_id",
				multi_data: 1
			};
			return dd
		}
		...
		function c9(da) {
			if (da) {
				return {
					...
					sw_version: da.cr_version.split("-")
						.join("‑"),
					hw_version: da.hw_version,
	...
	function am() {
		...
		function c8(da, db) {
			...
			var dc = {
				...
				data_volume_limit_switch: da.dataLimitChecked
			};
	...
	function aK() {
		...
		function c8(dd) {
			return {
				...
				notCallback: true,
				unlock_network_code: dd.unlock_network_code
			}
		}
	...
	function bK() {
		...
		function c8(dd) {
			return {
				...
				notCallback: true,
				sim_unlock_code: dd.sim_unlock_code
			}
		}
	...
	function aZ() {
		...
		function c8(da, db) {
			return {
				cmd: "sim_lock_status,cstm_webui_unlocksim",
				multi_data: "1"
			}
		}
		...
		function c9(da) {
			if (da) {
				...
				return {
					sim_lock_status: da.sim_lock_status,
					cstm_webui_unlocksim: dc
				}
			} else {
				return V
			}
	...
	function c1() {
		...
		function c8(db) {
			return {
				...
				upgrade_notice_flag: db.upgrade_notice_flag,
				notCallback: true
			}
		}
	...
	function bc() {
		...
		function c8(db) {
			return {
				...
				wifi_sta_connection: db.ap_station_enable
			}
		}
	...
	function aj() {
		...
		function c8(dj) {
			...
			if (dj.profileName == "") {
				da = "add";
				var df = (jQuery.fn.jquery + Math.random())
					.replace(/\D/g, "");
				dj.profileName = df;
				dk.push({
					profileName: df,
	...
	function x() {
		...
		function c9(da) {
			if (da) {
				return {
					currentMode: da.syslog_mode,
					debugLevel: da.debug_level
				}
			} else {
				return V
			}
	...
	function bQ() {
		...
		function c8(da) {
			return {
				...
				mgmt_quicken_power_on: da.fastbootEnabled
			}
		}
	...
	function bk() {
		...
		function c9(da) {
			if (da) {
				if (aF.UPGRADE_TYPE == "OTA") {
					return {
						is_mandatory: da.is_mandatory == "1"
					}
				} else {
					return {
						is_mandatory: da.fota_new_version_state == "has_critical"
					}
				}
			} else {
				return V
	...
	function cq() {
		return by(arguments, aF.currentUserInChildGroup == false ? {} : {
			errorType: "no_auth"
		}, c8, c9, null, true);
	...
	function s(c8) {
		if (typeof aF.currentUserInChildGroup == "undefined") {
			...
			return {
				result: typeof da != "undefined"
			}
		}
		return {
			result: aF.currentUserInChildGroup
		}
	}
	...
	function bP() {
		...
		function c8(da, db) {
			var dc = {
				goformId: "SAVE_TSW",
				openEnable: da.openEnable,
				closeEnable: da.closeEnable
			};
			if (da.openEnable == "1") {
	...
	function cQ() {
		...
		function c9(da) {
			return {
				update_type: da.update_type ? da.update_type : "mifi_fota"
			}
		}
	}
...
define("adm_lan", "jquery knockout set service".split(" "), function(c, b, q, s) {
	...
	function n() {
		...
		w.saveDnsPara = function() {
			showLoading();
			var y = {
				dnsServerMode: w.dnsServerMode(),
				dnsPrimary: w.dnsPrimary()
			};
	...
	c.validator.addMethod("dhcp_check", function(v, w, x) {
		var u = x == "start" ? c("#txtDhcpIpPoolStart")
			.val() : c("#txtDhcpIpPoolEnd")
			.val();
		var t = k(c("#txtIpAddress")
			.val(), c("#txtSubnetMask")
			.val(), u);
	...
	c.validator.addMethod("ipRange", function(u, w, x) {
		...
		var t = g(u, c("#txtSubnetMask")
			.val(), c("#txtDhcpIpPoolStart")
			.val(), c("#txtDhcpIpPoolEnd")
			.val(), v);
		return t != 2
	});
...
define("adm_others", "jquery knockout set service underscore".split(" "), function(c, a, r, t, s) {
	...
	function o() {
		...
		c("#swithMode")
			.val(H.sim_auto_switch_enable);
		C.swithMode = a.observable(c("#swithMode")
			.val());
		if (H.sim_auto_switch_enable == 0) {
			c("#swithCard")
				.val(H.sim_current_type)
		} else {
			c("#swithCard")
				.val(H.sim_default_type)
		}
		...
		if (C.imeiFuncEnable() == "2") {
			...
			if (C.imeiFuncEnable() == "1") {
				...
				c(".setImei")
					.attr("id", "removeSetImei");
				c("#removeSetImei")
					.css("display", "none")
			}
		}
		var A = t.getTTLValue();
		document.getElementById("daylight_saving_TTL_select")
			.value = A.ttlValue;
		C.ttlFuncEnable = a.observable(v.ttlFuncEnable);
		if (C.ttlFuncEnable() == "2") {
			c(".setTTLForm")
				.attr("id", "rootSetTTLForm")
		} else {
		...
		var u = checkCableMode(t.getOpMode()
			.blc_wan_mode);
		var v = t.getExtraFunc();
		...
		if (r.HAS_PARENTAL_CONTROL) {
			E = t.checkCurrentUserInChildGroup()
				.result
		}
		C.currentUserInChildGroup = a.observable(E);
		var F = t.getFastbootSetting();
		C.fastbootEnableFlag = a.observable(r.RJ45_SUPPORT ? (F.need_sim_pin != "yes" && t.getRj45PlugState()
			.rj45_plug == "wan_lan_off") : F.need_sim_pin != "yes");
		C.fastbootSetting = a.observable(F.fastbootEnabled);
		addInterval(function() {
			C.fastbootEnableFlag(r.RJ45_SUPPORT ? (F.need_sim_pin != "yes" && t.getRj45PlugState()
				.rj45_plug == "wan_lan_off") : F.need_sim_pin != "yes")
		}, 1000);
				...
				if (K) {
					c("#txtCmdRsp")
						.append("\n" + c("#txtAtCmd")
							.val() + K.result);
					c("#txtCmdRsp")
						.scrollTop(c("#txtCmdRsp")[0].scrollHeight)
				} else {
					errorOverlay()
				}
			})
		...
		C.saveFastBoot = function() {
			showLoading();
			var I = {
				fastbootEnabled: C.fastbootSetting(),
				need_hard_reboot: F.need_hard_reboot
			};
		...
		C.saveSIMSwitch = function() {
			...
			var L = c("#swithCard")
				.val();
			var I = c("input:radio[name='switch_running']:checked")
				.val();
			var K = {
				sim_mode: J,
				sim_type: L,
		...
		C.apply = function() {
			...
			var K = {
				...
				timezone: C.currentTimeZone()
					.split("_")[0],
				sntp_timezone_index: C.currentTimeZone()
					.split("_")[1],
		...
		C.sim_unlock = function() {
			t.unlockSim({
				sim_unlock_code: C.unlock_sim_code()
			}, function(I) {
				if (I && I.result == "success") {
					C.SIMislock(false);
					C.enableFlag(true);
		...
		function w() {
			...
			!C.isOther0() && c("#sntp_server0")
				.find(".error")
				.hide();
			!C.isOther1() && c("#sntp_server1")
				.find(".error")
				.hide();
			!C.isOther2() && c("#sntp_server2")
				.find(".error")
				.hide()
		}
	}
...
define("adm_quick_set", "jquery knockout set service underscore CryptoJS".split(" "), function(g, o, c, k, m, l) {
	...
	function f() {
		...
		K.apn = o.observable(K.currAPN()
			.wan_apn);
		K.ipv6_apn = o.observable(K.currAPN()
			.ipv6_wan_apn);
		K.ipv6_selectedAuthMode = o.observable(K.currAPN()
			.ipv6_ppp_auth_mode);
		K.ipv6_username = o.observable(K.currAPN()
			.ipv6_ppp_username);
		K.ipv6_password = o.observable(K.currAPN()
			.ipv6_ppp_passtmp);
		K.profileName = o.observable(K.currAPN()
			.m_profile_name);
		K.selectedAuthMode = o.observable(K.currAPN()
			.ppp_auth_mode);
		K.username = o.observable(K.currAPN()
			.ppp_username);
		K.password = o.observable(K.currAPN()
			.ppp_passtmp);
		K.ipType = o.observable(K.currAPN()
			.pdp_type == "IP" ? "IP" : K.currAPN()
			.ipv6_pdp_type);
		...
		function t() {
			showLoading();
			var N = c.IPV4V6_SUPPORT && K.currAPN()
				.pdp_type == "IPv4v6";
			var O = "";
...
define("main", "set service knockout underscore jquery statusBar echarts".split(" "), function(k, f, e, l, h, g, c) {
	...
	var n = {
		...
		initShownStatus: function(q) {
			this.initStatus = {};
			var r = q.ipv6PdpType.toLowerCase()
				.indexOf("v6") > 0;
			if (k.RJ45_SUPPORT) {
		...
		refreshStationInfo: function(q) {
			q.wirelessDeviceNum(f.getStatusInfo()
				.wirelessDeviceNum);
			if (a % 10 == 2) {
	...
	function p() {
		...
		if (w.hasRj45) {
			var q = checkCableMode(f.getOpMode()
				.blc_wan_mode);
		...
		w.cStatus = e.computed(function() {
			if (w.connectStatus()
				.indexOf("_connected") != -1) {
				return b.CONNECTED
			} else {
				if (w.connectStatus()
					.indexOf("_disconnecting") != -1) {
					return b.DISCONNECTING
				} else {
					if (w.connectStatus()
						.indexOf("_connecting") != -1) {
						return b.CONNECTING
					} else {
		...
		w.imsi = e.observable("");
		w.ssid = e.observable("");
        w.resideBand = e.observable("");
        w.maxAccessNum = e.observable("");
        w.ipAddress = e.observable("");
        w.wanIpAddress = e.observable("");
        w.ipv6WanIpAddress = e.observable("");
		...
		w.wireDeviceNum = e.observable(f.getAttachedCableDevices()
			.attachedDevices.length);
		w.wirelessDeviceNum = e.observable(f.getStatusInfo()
			.wirelessDeviceNum);
		...
		function x() {
            ...
            w.resideBand(verifyDeviceInfo(y()));
            w.maxAccessNum(verifyDeviceInfo(z.max_access_num));
            w.ipAddress(verifyDeviceInfo(z.ipAddress));
            w.wanIpAddress(verifyDeviceInfo(z.wanIpAddress));
            w.ipv6WanIpAddress(verifyDeviceInfo(z.ipv6WanIpAddress));
		...
		function y() {
			...
			if (checkCableMode(f.getOpMode()
				.blc_wan_mode) || z.reside_band == "band_invalid") {
				return "— —"
			} else {
		...
		f.getSignalStrength({}, function(A) {
			...
			h("#fresh_signal_strength")
				.text(z);
			h("#fresh_rsrq_strength")
				.text(A.rsrq);
			h("#fresh_sinr_strength")
				.text(A.sinr);
			h("#fresh_pci_strength")
				.text(A.pci);
			if (u) {
				h("#popoverSignalTxt")
					.text(z)
...
define("opmode_popup", "knockout service jquery set underscore".split(" "), function(d, a, f, c, b) {
	function e() {
		...
		j.changeOpMode = function() {
			var k = f('input:radio[name="opMode"]:checked')
				.val();
			var l = "";
			if (k == h) {
	...
	function m() {
		...
		if (window.location.hash == "#entry") {
			...
			if (o.hasClass("loginBackgroundBlue")) {
				...
				o.removeClass("loginBackgroundBlue")
					.height("auto")
			}
		}
	}
	...
	function h() {
		if (window.location.hash != e) {
			...
			if (q.length == 0) {
				...
				if (e == "#entry") {
					f("#indexContainer")
						.addClass("login-page-bg");
					b.rebuild()
				} else {
					f("#indexContainer")
						.removeClass("login-page-bg")
				}
...
define("statusBar", "knockout jquery underscore service set menu tooltip".split(" "), function(V, G, ac, g, L, W, I) {
	...
	function M() {
		...
		ai.updateType = V.observable(g.getUpdateType()
			.update_type);
	...
	function y(au, ak, an) {
		...
		G(".bubbleItem:not(.report)", "#buttom-bubble")
			.remove();
		G.tmpl("newMessagePopTmpl", ai)
			.appendTo("#buttom-bubble");
		if ((window.location.hash == "#msg_main" || window.location.hash == "#msg_list") && an == "1") {
			...
			if (at && at.length > 0) {
				...
				at.find(".smslist-item-checkbox p.checkbox")
					.attr("id", au.id);
				at.find(".smslist-item-checkbox input:checkbox")
					.val(au.id)
					.attr("id", "checkbox" + au.id);
				if (!ak) {
					var ah = at.find(".smslist-item-total-count")
						.text();
					ah = Number(ah.substring(1, ah.length - 1));
					at.find(".smslist-item-total-count")
						.text("(" + (ah + 1) + ")");
					if (!L.currentChatObject || L.currentChatObject != getLastNumber(au.number, L.SMS_MATCH_LENGTH)) {
						var aj = at.find(".smslist-item-new-count")
							.removeClass("hide");
						if (aj && aj.text()
							.length > 0) {
				...
				if (at.find(".smslist-item-draft-flag")
					.length > 0) {
					if (L.currentChatObject && L.currentChatObject == getLastNumber(au.number, L.SMS_MATCH_LENGTH)) {
						at.find(" td:nth-child(2)")
					...
					var ae = at.find(".smslist-item-msg")
						.text(au.content);
					ae.closest("td")
						.prop("title", au.content);
					at.find("span.clock-time")
						.text(au.time);
					if (L.currentChatObject && L.currentChatObject == getLastNumber(au.number, L.SMS_MATCH_LENGTH)) {
						ae.closest("tr")
							.removeClass("font-weight-bold")
				...
				at.find(".smslist-item-repeat span")
					.die()
					.click(function() {
			...
			if (ag) {
				...
				if (ad && ad.length > 0) {
					G(".J_content pre", ad)
						.html(dealContent(au.content));
					G(".time .smslist-item-time", ad)
						.text(au.time);
					G(".smslist-item-repeat", ad)
						.die()
						.click(function() {
							forwardClickHandler(au.id)
						});
					G(".smslist-item-delete", ad)
						.die()
						.click(function() {
					...
					G("#smsOtherTmpl")
						.tmpl(au)
						.appendTo("#chatlist");
					G(".clear-container", "#chatpanel")
						.animate({
							scrollTop: G("#chatlist")
								.height()
						})
				}
			...
			G.each(ae, function(ag, ah) {
				if (G.inArray(ah.number, ad) == -1) {
					...
					window.setTimeout(function() {
						...
						G(".report", "#buttom-bubble")
							.remove();
						G.tmpl("newMessagePopTmpl", ak)
							.appendTo("#buttom-bubble")
					}, 100)
				}
			})
	...
	function R() {
		if (L.PRODUCT_TYPE == "DATACARD") {
			G("#statusBar")
				.addClass("padding-right-90");
			G("#language")
				.addClass("data-card-language")
		}
		var ag = G("<img />")
			.attr("src", "pic/res_alert.png");
		var af = G("<img />")
			.attr("src", "pic/res_confirm.png");
		var aj = G("<img />")
			.attr("src", "pic/res_info.png");
		window.setTimeout(function() {
			var al = new M();
			...
			window.setInterval(function() {
				...
				if (al.isLoggedIn() == true && !(G("#progress")
					.is(":visible")) && ao.defaultWanName != "") {
			...
			function ak(an) {
				g.getSmsCapability({}, function(ao) {
					...
					if (ao.nvTotal != 0 && ao.nvUsed >= ao.nvTotal) {
						G("#sms_unread_count")
							.attr("tipTitle", "sms_capacity_is_full");
						ap = true
					} else {
						if (ao.nvTotal != 0 && ao.nvUsed + 5 >= ao.nvTotal) {
							G("#sms_unread_count")
								.attr("tipTitle", "sms_capacity_will_full");
							ap = true
						} else {
							G("#sms_unread_count")
								.attr("tipTitle", "sms_unread_count")
						}
					}
		...
		checkTrafficLimitAlert = function(an, ao) {
			...
			var ak = L.AP_STATION_SUPPORT ? g.getStatusInfo()
				.ap_station_enable : "undefined";
			var ap = ab(ao);
			var am = G("#confirm-container:visible")
				.length > 0;
		...
		function ad(ak) {
			G("#statusItemSimStatus")
				.attr("tipTitle", "sim_status_" + ak.simStatus);
			if (ak.wifiStatus) {
				if (ak.deviceSize == 0) {
					G("#wifi_status")
						.attr("tipTitle", "wifi_status_on")
				} else {
					G("#wifi_status")
						.attr("tipTitle", "wifi_status" + ak.deviceSize)
				}
			} else {
				G("#wifi_status")
					.attr("tipTitle", "wifi_status_off")
			}
		}
		...
		getNetworkType = function(al) {
			...
			if (ak == "limited_service" || ak == "no_service") {
				G("#networkType", "#statusBar")
					.attr("data-trans", "network_type_" + ak);
		...
		if (L.HAS_SMS) {
			G(".bubbleItem", "#buttom-bubble")
				...
				.live("mouseout", function() {
					...
					Y["m" + ak] = ak;
					al.attr("id", "m" + ak);
					G(".bubbleItem h3 a.bubbleCloseBtn", "#buttom-bubble")
						.data("targetid", "m" + ak)
				});
			G(".bubbleItem h3 a.bubbleCloseBtn", "#buttom-bubble")
				.die()
...
define("status_traffic_alert", "jquery knockout service statusBar echarts".split(" "), function(h, n, k, f, e) {
	...
	var j = {
		...
		isFormEditable: function(s) {
			...
			if (r || p) {
				h(".border-color-transition:visible")
					.addClass("attention-focus");
				addTimeout(function() {
					h(".border-color-transition:visible")
						.removeClass("attention-focus")
				}, 1500);
				return true
			} else {
				var q = false;
	...
	function c() {
		...
		function Z() {
			p.alertDataReach(j.getEle("editAlertData")
				.data("oldValue"));
			p.viewEditAlertData(false)
		}

		function L() {
			j.getEle("editAlertData")
				.data("oldValue", p.alertDataReach());
			p.viewEditAlertData(true)
		}

		function q() {
			if (j.getEle("alertDataReach")
				.valid()) {
				p.viewEditAlertData(false)
			}
		}
		...
		function W() {
			p.alertTimeReach(j.getEle("editAlertTime")
				.data("oldValue"));
			p.viewEditAlertTime(false)
		}

		function K() {
			j.getEle("editAlertTime")
				.data("oldValue", p.alertTimeReach());
			p.viewEditAlertTime(true)
		}

		function U() {
			if (j.getEle("alertTimeReach")
				.valid()) {
				p.viewEditAlertTime(false)
			}
		}
		...
		function E() {
			j.getEle("editTotalData")
				.data("oldValue", p.limitDataMonth());
			j.getEle("selectedDataUnit")
				.data("oldValue", p.selectedDataUnit());
			p.viewEditTotalData(true)
		}

		function s() {
			if (j.getEle("limitDataMonth")
				.valid()) {
		...
		function Q() {
			p.limitDataMonth(j.getEle("editTotalData")
				.data("oldValue"));
			p.selectedDataUnit(j.getEle("selectedDataUnit")
				.data("oldValue"));
			p.viewEditTotalData(false)
		}

		function P() {
			p.limitTimeMonth(j.getEle("editTotalTime")
				.data("oldValue"));
			p.viewEditTotalTime(false)
		}

		function D() {
			j.getEle("editTotalTime")
				.data("oldValue", p.limitTimeMonth());
			p.viewEditTotalTime(true)
		}

		function ab() {
			if (j.getEle("limitTimeMonth")
				.valid()) {
				p.viewEditTotalTime(false)
			}
		}
		...
		function G() {
			p.dataUsed(j.getEle("editUsedData")
				.data("oldValue"));
			p.selectedDataUsedUnit(j.getEle("selectedDataUsedUnit")
				.data("oldValue"));
			j.getEle("editUsedDataCancel")
		...
		function x() {
			j.getEle("editUsedData")
				.data("oldValue", p.dataUsed());
			j.getEle("selectedDataUsedUnit")
				.data("oldValue", p.selectedDataUsedUnit());
			p.dataUsed(p.dataUsed());
			p.viewEditUsedData(true)
		}

		function aa() {
			if (j.getEle("dataUsed")
				.valid()) {
				b = true;
				p.viewEditUsedData(false)
			}
		...
		function C() {
			p.usedTime(j.getEle("editUsedTime")
				.data("oldValue"));
			p.viewEditUsedTime(false)
		}

		function v() {
			j.getEle("editUsedTime")
				.data("oldValue", p.usedTime());
			p.viewEditUsedTime(true)
		}

		function O() {
			if (j.getEle("usedTime")
				.valid()) {
...
define("tooltip", ["jquery"], function(b) {
	...
	function c() {
		b(".statusItem", "#statusBar")
			.each(function(d, f) {
				var e = b(this);
				e.attr("tipTitle", e.attr("title"))
					.removeAttr("title")
			})
			.hover(function() {
				var e = b(this);
				...
				var d = b("<div>")
					...
					.hide()
					.append(e.attr("i18n") ? b.i18n.prop(f) : f);
				if (e.attr("i18n")) {
					d.attr("data-trans", f)
						.attr("id", "tooltip_" + e.attr("id"))
				}
				var g = a(e, d, {
...
define("menu", "set service knockout underscore jquery".split(" "), function(p, r, q, u, j) {
	...
	function h(x, w) {
		...
		if (v) {
			...
			j(".menu-" + x + "-level." + v.hash.substring(1))
				.addClass("active")
		}
	}
	...
	function t() {
		...
		if (w.level == 3) {
			...
			j(".menu-three-level")
				.removeClass("active");
			j(".menu-three-level." + w.hash.substring(1))
				.addClass("active")
		}
	}
	...
	function c() {
		...
		x.changeMenu = function(z) {
			...
			if (A.length == 0) {
				j("#container")
					.addClass("fixContainerWidth")
			} else {
				j("#container")
					.removeClass("fixContainerWidth")
			}
			x.secondMenu(A);
			return true
		};
